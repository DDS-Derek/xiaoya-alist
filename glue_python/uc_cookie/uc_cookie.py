...
if __name__ == '__main__':
    ...
    try:
        if args.qrcode_mode == 'web':
            wait_status = threading.Thread(target=poll_qrcode_status, args=(stop_event,token,True,))
            wait_status.start()
            app.run(host='0.0.0.0', port=34256)
        elif args.qrcode_mode == 'shell':
            wait_status = threading.Thread(target=poll_qrcode_status, args=(stop_event,token,False,))
            wait_status.start()
